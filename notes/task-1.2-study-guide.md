...
offset代表当前Tensor的数据，从其底层Storage的第几个元素开始的。
✅ 订正一个单位细节：这个项目里 `_offset` 以**字节**计（`memory()` 返回 `std::byte *`），不是元素个数。
比如，对于(4,4)的Tensor，切出第一行到第三行，sliced_tensor不会申请新的显存去拷贝元素，直接复用。
那么，靠offset就可以跳过第0行了。

即：Physical Index = _offset + ∑^{ndim-1}_{k=0}(i_k • strides[k])

所以，答案是：不影响。只有跨列切片(tensor[:,::2], 隔一个取一个)，导致elem之间有空元素，张量才不连续
✅ 补一个反例：“只有…才”过于强了——permute 制造不出空洞，但同样不连续：(3,4) strides (4,1) 转置后 (4,3) strides (1,4)，元素一个不少，只是**逻辑遍历顺序和物理摆放不再一致**。准确表述：不连续 = 有空洞 **或** 顺序错乱。
...
**连续性看的是“元素在物理内存里是否紧密挨着且和物理内存中的数据和逻辑保持一致”，而不是看 strides 数组里的数字凑得齐不齐。**

对于一个N维向量，若其在内存中连续，则：
1. 最后一个维度(N-1)的步长固定为1（Stride\[N-1\] = 1)
2. 前面任意维度的步长，等于其右侧相邻维度步长乘以右侧相邻维度的大小。（Stride\[i\] = Stride\[i+1\]•Shape\[i+1\])

展开可得：Stride\[i\]（当前Strides） = ∏^{N-1}_{k=i+1} Shape\[k\]（期望Strides）

- 0 维张量（shape 为空）和空张量（某维为 0）应该返回什么？
都是连续，因为Shape为空就是标量，Shape中有一个维度为0，根本没有任何实际数据存储在物理内存中。
✅ 正确。（顺带回应上面 size-1 的问题：你写的展开公式是**严格版**判据；PyTorch 语义里 size-1 维的 stride 是 don't-care，(2,1,3) strides (3,999,1) 也算连续。想兼容就在循环里加 `if (shape[d] == 1) continue;`，本项目测试没有 size-1 用例，严格版也能过。）

- strides 的类型是 `ptrdiff_t`（有符号）。负 stride 意味着什么？这个项目支持吗？（看 `test/test_utils.py:132-133`。）

目前不支持,pytorch也不支持。neg strides，是不是描述翻转视图？

- 如果 `isContiguous()` 直接 `return true;`，`test_tensor.py` 能过吗？为什么仍然不允许？
  （提示：看 18/28/38/48 行断言的永远是哪个张量；再想想作业 #2 的算子为什么敢对 `data()` 做扁平指针算术。）
  并不能，因为这个是要跟Pytorch做比较
✅ 订正：恰恰**能**过。18/28/38/48 行断言的永远是原始张量 `llaisys_tensor`（连续，torch 侧也是 True），view/permute/slice 的产物从没被断言过。所以 `return true;` 今天能蒙混过关——不允许它的原因是**语义造假**：1.3 view 的合法性检查、contiguous()/reshape()、作业 #2 算子的扁平索引，全都默认这个函数说真话。
...
Stride\[i\]（当前Strides） = ∏^{N-1}_{k=i+1} Shape\[k\]（期望Strides）
- [ ] 为什么判断顺序要**从最后一维往前**推，而不是反过来？
因为最后一维的Stride有明确的基准，是一个起点。标准contiguous的最后一维是0，然后从这里根据Shape去推，就是获得contiguous的strides。
一个shape，假设其为contiguous，则strides唯一，且最后一位为0。可以通过shape反推。
✅ 订正一个笔误：基准值是 **1 不是 0**——最后一维相邻元素相隔 1 个元素，期望 stride 从 1 开始累乘（若是 0，乘出来全是 0）。其余推理正确。
- [ ] 为什么 `_offset` 和连续性无关？
连续性无关起始地址
- [ ] permute 之后为什么一定不连续？（有例外吗？想想 size-1 维或对调后形状不变的特殊情况——不用实现，理解即可。）
不清楚诶，但是我大概鞥理解到是，元素在物理地址中的存储和逻辑不一致了。
✅ 直觉对。具体化：permute 把 shape 和 strides **一起重排**，(3,4) strides (4,1) → permute(1,0) → shape (4,3) strides (1,4)；而 (4,3) 紧凑布局的期望 strides 是 (3,1)，对不上 → 不连续。
例外：对调的维度中若含 size-1 维，结果可能仍连续（该维 stride 不影响布局）；恒等 permute 当然也连续。所以“一定不连续”严格说是错的，但“几乎一定”。
- [ ] slice 之后可能连续吗？（提示：沿 dim 0 切 vs 沿最后一维切，有区别。）
有可能吧，只要切掉dim 0 完全OK，每行内部是连着的，只动Offset。
✅ 对，再精确一点：对**连续**张量，只有沿**第 0 维**切才保持连续。切中间维也不行：(3,4,5) 切 dim1 [1:3) 得 shape (3,2,5)，strides 仍为 (20,5,1)，期望却是 (10,5,1) → 不连续（每 20 个元素里取一段 10 个的连续段，段间有空洞）。
